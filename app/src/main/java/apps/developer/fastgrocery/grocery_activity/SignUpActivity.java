...
public class SignUpActivity extends AppCompatActivity {
    ...
    private void RegisterUser() {
        ...
        jsonObject.addProperty("name", edUsername.getText().toString());
        jsonObject.addProperty("hno", edHoousno.getText().toString());
        jsonObject.addProperty("society", edSociety.getText().toString());
        jsonObject.addProperty("area", areaSelect);
        jsonObject.addProperty("landmark", edLandmark.getText().toString());
        jsonObject.addProperty("pincode", edPinno.getText().toString());
        jsonObject.addProperty("email", edEmail.getText().toString());
        jsonObject.addProperty("mobile", edAlternatmob.getText().toString());
        jsonObject.addProperty("password", edPassword.getText().toString());
        jsonObject.addProperty("imei", Utiles.getIMEI(SignUpActivity.this));

        apiService.getRegister(jsonObject).enqueue(new Callback<RegisterResponse>() {
            @Override
            public void onResponse(Call<RegisterResponse> call, Response<RegisterResponse> response) {
                custPrograssbar.ClosePrograssBar();

                Toast.makeText(SignUpActivity.this, "" + response.body().getResponseMsg(), Toast.LENGTH_LONG).show();
        ...
        apiService.getArea(jsonObject).enqueue(new Callback<AreaResponseData>() {
            ...
            public void onResponse(Call<AreaResponseData> call, Response<AreaResponseData> response) {
                ...
                if (response.body().getResult().contains("true")){
                    ...
                    ArrayAdapter<String> dataAdapter = new ArrayAdapter<String>(SignUpActivity.this, android.R.layout.simple_dropdown_item_1line, Arealist);
